...
void Stringf::Print(CHA item) {
  CHA* buffer = reinterpret_cast<CHA*>(buffer_);
...
void Stringf::Hex(CHA item, ISW count) {
  *reinterpret_cast<CHA*>(buffer_) = item;
  type_ = cCHA;
  count_ = -count;
}
#if USING_UTF16 == YES_0
...
void Stringf::Hex(IUB item, ISW count) {
  *reinterpret_cast<IUB*>(buffer_) = item;
  type_ = cIUB;
  count_ = -count;
}
...
void Stringf::Hex(IUC item, ISW count) {
  *reinterpret_cast<IUC*>(buffer_) = item;
  type_ = cIUC;
}
...
void Stringf::Hex(IUD item, ISW count) {
  *reinterpret_cast<IUD*>(buffer_) = item;
  type_ = cIUD;
  count_ = -count;
}
