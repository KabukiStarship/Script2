...
Char* TPrintSocket(Char* cursor, Char* stop, const void* begin,
  ...
  size += num_bytes;
  cursor = TPrint<Char> (cursor, stop, StringSocketHeader ());
  cursor = TPrint<Char> (cursor, stop, StringSocketBorder ());
...
struct TUTF {
  Char *begin,  //< Start address. @todo rename start.
      *stop;    //< Stop address.
...
template <typename Char = CH1, typename Size = SI4>
inline Char* TStringStop(void* begin) {
...
class TSTR {
 ...
 private:
  TCSTR<Size, Char>* obj_;  //< Pointer to an ASCII CH1*.
};
...
template <typename Char, SI4 kSize_>
Char* TStrandInit (TSocket<SI4, kSize_>& socket) {
  return TStringInit<SI4, Char> (socket.Begin<> (), kSize_);
}

enum {
  kStrandSizeMin = sizeof (SI4) + 2 * sizeof (CH4), //< Strand min size.
};
...
SI4 TStrandFactory (CObject& obj, SIW function, void* arg, BOL using_heap) {
  ...
  case kFactoryGrow: {
    ...
    size = size << 1;  ///<< 1 to * 2
    temp = obj.begin;
    obj.begin = TObjNew<SI4> (size, kStrandSizeMin);
    if (using_heap)
      delete[] temp;
    else
      obj.factory = TStrandFactory<Char, kHeap>;
...
buffer, including the nil-term char.
...
@endcode */
template <typename Char = CH1, SI4 kCount_ = kStrandCountMax,
...
kCount = (kCountLB >= kLengthUpperLimit) ? kLengthUpperLimit - 1 :
  kCountLB,
    ...
    /* Prints a CH1 to the strand.
    @return A UTF. */
    TUTF<Char>& Print (const Char* item) {
      Char *cursor = utf_.begin, *stop = TStringStop<Char> (obj_.Begin ());
      cursor = ::_::Print (cursor, stop, item);
      if (!cursor) {
        do {
    ...
    inline Char* Start () { return TStringStart<Char, SI4> (obj_.Begin ()); }

    /* Returns the stop of the obj. */
    inline Char* Stop () { return TStringStop<Char, SI4> (obj_.Begin ()); }
    ...
    void Print () {
      ...
      ::_::Print ('>');
      obj_.Print ();
      ::_::Printf ("TUTF<CH%C>{0x%p, 0x%p}", sizeof (Char), utf_.begin, utf_.stop);
      ::_::PrintSocket (socket_.Begin (), socket_.End ());
    }
  ...
  private:
    ...
    void PrintConstants () {
      PRINTF (
        "\n\nutf_.begin:0x%p utf_.stop:0x%p"
        "\nkSizeMax:%i kLengthMax:%i kCountMin:%i kCount:%i kSize:%i\n\n",
        utf_.begin, utf_.stop, kSizeMax, kLengthUpperLimit, kCountMin, kCountLB, kCount, kSize);
    }
};
...
SI4 TStrandFactory (CObject& obj, SIW function, void* arg) {
  return TStrandFactory (obj, function, arg, kUsingHeap_);
}
...
template <typename Char = CH1>
inline ::_::TUTF<Char>& operator<<(::_::TUTF<Char>& utf, const CH1* string_) {
...
@param  value   The value to utf. */
template <typename Char = CH1>
inline ::_::TUTF<Char>& operator<<(::_::TUTF<Char>& utf, const CH2* string_) {
  return utf.Set (::_::TPrint<Char> (utf.begin, utf.stop, string_));
}
...
@param  value   The value to utf. */
template <typename Char = CH1>
inline ::_::TUTF<Char>& operator<<(::_::TUTF<Char>& utf, const CH4* string_) {
  return utf.Set(::_::TPrint<Char>(utf.begin, utf.stop, string_));
}
