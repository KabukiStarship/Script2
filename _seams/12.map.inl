...
template <typename D, D kDomainMin_, D kDomainMax, typename SIZ,
          SIZ kCodomainMin_, SIZ kCodomainMax_, SIZ cSize_>
void TestMap() {
  D_COUT(Linef("\n\n\n\n\n\n+---\nTesting AMap<D")
         << sizeof(D) << ",SIZ" << CH1('0' + sizeof(SIZ))
         << Linef(">\n+---\n\n"));

  AMap<D, SIZ, cSize_> map;
  D_COUT_OBJ(map);
  ...
  for (SIZ i = 0; i < cSize_; ++i) {
    D d = Random(kDomainMin_, kDomainMax);
    domain[i] = d;
    D_COUT("\n" << i << ".) " << d);
  }

  for (SIZ i = 0; i < cSize_; ++i) map.Add(domain[i]);

  D_COUT(Headingf("Searching the domain for each domain[i]..."));
  for (SIZ i = 0; i < cSize_; ++i) A_AVOW_INDEX(i, SIZ(map.Find(domain[i])), i);
