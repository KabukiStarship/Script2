...
ASCII Data Types are manipulated using the ASCII Factory, or AsciiFactory, function. The AsciiFactory abstracts away the memory handler using a single C-style function pointer with the following prototype:
...
enum AsciiFactoryFunctions {
  ...
  kFactoryInfo = 4,    //< Returns an info string about the data type.
};
```
